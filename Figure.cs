...
namespace OOP_drow
{
    ...
    class FigureControl
    {
        ...
        public void DrawSh(Graphics gr)
        {
            ...
            if (cs != null)
            {
                ...
                gr.DrawRectangle(p, cs.sp.X, cs.sp.Y, cs.width, cs.height);

            }
        }

        public void DrawCP(Graphics gr, Figures s)
        {
            //gr.FillRectangle(new SolidBrush(p.Color),)
        }
    ...
    class Figures
    {
        ...
        protected List<Figures> sObjectListOfFigures = new List<Figures>();
        ...
        {
            foreach (Figures s in sObjectListOfFigures)
            {
                if (s.HasPoint(p))
                    return true;
            }
    ...
    class sRect : Figures
    {
        public sRect()
        {
            ...
            sObjectListOfFigures.Add(new Line());
            sObjectListOfFigures.Add(new Line());
            sObjectListOfFigures.Add(new Line());
            sObjectListOfFigures.Add(new Line());
            SetSPEP(sp,ep);
          ...
          //  SetText("Проверка fjdk dsfj dsj dsfjksf  dsfj \n dsjffh dsfhjds jdsfh jdshfjks lfkdl lsakdlsakdls lsakdlsadladl lsakdlsa lsa lsakdl; lsakda\n dsfjsf  kljdsfj dsfjs  ");
        }
        ...
        public override void SetSPEP(Point sp1, Point ep1)
        {
            ...
            sObjectListOfFigures[0].SetSPEP(sp, new Point(sp.X, ep.Y));
            sObjectListOfFigures[1].SetSPEP(sp,new Point(ep.X, sp.Y));
            sObjectListOfFigures[2].SetSPEP(new Point(ep.X, sp.Y),new Point(ep.X, ep.Y));
            sObjectListOfFigures[3].SetSPEP(new Point(sp.X, ep.Y), new Point(ep.X, ep.Y));
        }
        public override void DrawSh(Graphics gr)
        {
            base.DrawSh(gr);
    ...
    class sObject : Figures
    {
        

        public sObject()
        {
            sObjectListOfFigures.Add(new sRect());
            sObjectListOfFigures.Add(new Line());
            SetSPEP(sp, ep);
         }
        public override void SetSPEP(Point sp1, Point ep1)
        {
            base.SetSPEP(sp1, ep1);
            sObjectListOfFigures[0].SetSPEP(sp, new Point(ep.X, sp.Y + (ep.Y - sp.Y) / 2));
            sObjectListOfFigures[1].SetSPEP(new Point(sp.X+(ep.X - sp.X) / 2, sp.Y + (ep.Y - sp.Y) / 2), new Point(sp.X+(ep.X - sp.X) / 2, ep.Y));
        }

        public override void SetText(string s)
        {
            base.SetText(s);
            sObjectListOfFigures[0].SetText(s);
        }
    }
